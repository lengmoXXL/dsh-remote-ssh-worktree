...
import { MAX_GRACE_MS } from './subprocess.ts'
...
export async function dispatch(method: string, params: unknown, backends: Backends): Promise<unknown> {
  switch (method) {
    case 'fs.resolve': {
...
export function readHello(params: unknown): { readonly protocol: number; readonly token: string } {
