 ...
 * copy of each, and a second would break service identity. `ws` is a real
 * dependency and stays external too; it is resolved from this package's own
 * `node_modules`.
 *
 ...
 * names Lightning CSS hashes into a class map, and xterm's own plain
 * stylesheet, which is injected as written. Both become a module that attaches
 * one tagged `<style>` to the document the first time the factory runs, which
 * keeps the components on the same CSS Modules contract the in-repo client
 * packages use — local names, semantic `--dsw-*` tokens, no global leakage.
 *
 * @module dsh-terminal/build
 */
 ...
 * id must not. The character after the prefix records whether the file is a
 * CSS Module.
 */
const CSS_VIRTUAL_PREFIX = '\0dsh-terminal-css:'
