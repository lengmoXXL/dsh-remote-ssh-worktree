...
const ICON_NAMES = [
  ...
  'IconPlusOutline16', 'IconRefreshOutline16', 'IconRightUpOutline16', 'IconTrashOutline16',
...
function stubContext(): {
  ...
  const ctx = {
    ...
    slots: {
      ...
      register: (
        definition: { name: string; id: string; order: number; label?: () => string },
        component: unknown,
      ) => {
...
test('apply registers one settings section with a stable id and a bound locale', async () => {
  const { exports } = await loadBundle()
  ...
  assert.equal(registrations[0]?.id, 'dsh-remote-workspace')
...
test('apply registers both dictionaries under one namespace', async () => {
  ...
  assert.equal(locales[0]?.ns, 'dsh-remote-workspace')
...
test('the section renders its frame with the injected face threaded through', async () => {
  ...
  const { ctx, registrations } = stubContext()
  ;(exports['apply'] as (ctx: unknown) => void)(ctx)
  const component = registrations[0]?.component as (props: Record<string, unknown>) => unknown
